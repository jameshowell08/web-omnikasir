  ...
  return (
    <div className="flex flex-col justify-center items-center h-screen">
      <div className="flex flex-col border-2 p-8 rounded-xl min-w-sm">
        ...
        <input type="text" name="username" id="username" className="border-2 p-2 rounded-lg my-1" placeholder="Username" autoComplete="username" value={username} onChange={(e) => { onUsernameChange(e.target.value) }} />
        {usernameError !== "" && <p className="text-red-600">{usernameError}</p>}

        <label htmlFor="password" className="mt-3 font-bold">Password</label>
        <input type={showPassword ? "text" : "password"} name="password" id="password" className="border-2 p-2 rounded-lg my-1" placeholder="Password" autoComplete="current-password" value={password} onChange={(e) => { onPasswordChange(e.target.value) }} />
        ...
        <button className="mt-5 bg-black text-white p-2 rounded-xl max-w cursor-pointer disabled:bg-stone-300" onClick={loginBtnClick}>Login</button>
      </div>
    </div>
  );
}
